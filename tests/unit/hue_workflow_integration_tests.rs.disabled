use colors::{Color, ColorCollectionType, cli::HueArgs, config::Config};
use std::fs;
...
mod hue_workflow_integration_tests {
    ...
    fn test_complete_hue_analysis_workflow() {
        ...
        assert!(
            analysis_result.matched_colors.len() > 0,
            "No colors matched criteria"
        );
        assert!(
            analysis_result.matched_colors.len() <= 50,
            "Limit not respected"
        );
        assert!(
            analysis_result.total_processed > 0,
            "No colors were processed"
        );
        ...
        assert!(
            output_content.contains("target_hue: 120"),
            "Missing target hue in output"
        );
        assert!(
            output_content.contains("tolerance: 30"),
            "Missing tolerance in output"
        );
        assert!(
            output_content.contains("matched_colors:"),
            "Missing matched colors section"
        );
        ...
        for color in &analysis_result.matched_colors {
            ...
            assert!(
                hue_distance <= 30.0,
                "Color hue {} outside tolerance: distance = {}",
                hsl.hue,
                hue_distance
            );
            ...
            assert!(
                hsl.saturation >= 25.0 && hsl.saturation <= 75.0,
                "Color saturation {} outside range [25, 75]",
                hsl.saturation
            );

            // Lightness constraints
            assert!(
                hsl.lightness >= 30.0 && hsl.lightness <= 70.0,
                "Color lightness {} outside range [30, 70]",
                hsl.lightness
            );
        }

        println!(
            "✓ Complete workflow test passed: {} colors matched",
    ...
    fn test_multi_collection_workflow() {
        ...
        for collection in collections {
            ...
            let output_path = temp_dir
                .path()
                .join(format!("analysis_{}.json", collection));
            ...
            assert!(
                result.is_ok(),
                "Workflow failed for collection {:?}",
                collection
            );

            let analysis = result.unwrap();
            assert!(
                analysis.total_processed > 0,
                "No colors processed for collection {:?}",
                collection
            );
            ...
            match collection {
                ColorCollectionType::Css => {
                    assert!(
                        analysis.total_processed >= 140,
                        "CSS collection should have many colors"
                    );
                }
                ColorCollectionType::RalClassic => {
                    assert!(
                        analysis.total_processed >= 200,
                        "RAL Classic should have many colors"
                    );
                }
                ColorCollectionType::RalDesign => {
                    assert!(
                        analysis.total_processed >= 1500,
        ...
        for (i, args) in test_cases.into_iter().enumerate() {
            ...
            match result {
                Ok(analysis) => {
                    ...
                    println!(
                        "✓ Test case {} handled gracefully: {} colors",
                        i,
                        analysis.matched_colors.len()
                    );
                }
    ...
    fn test_workflow_performance_stress() {
        ...
        for (scenario_name, tolerance) in stress_scenarios {
            ...
            let args = HueArgs {
                ...
                min_saturation: if scenario_name == "many_constraints" {
                    Some(10.0)
                } else {
                    None
                },
                max_saturation: if scenario_name == "many_constraints" {
                    Some(90.0)
                } else {
                    None
                },
                min_lightness: if scenario_name == "many_constraints" {
                    Some(20.0)
                } else {
                    None
                },
                max_lightness: if scenario_name == "many_constraints" {
                    Some(80.0)
                } else {
                    None
                },
            ...
            let analysis = result.unwrap();
            println!(
                "✓ Stress test {}: {:?}, {} colors processed, {} matched",
                scenario_name,
                elapsed,
                analysis.total_processed,
                analysis.matched_colors.len()
    ...
    fn test_output_format_consistency() {
        ...
        for format in &formats {
            ...
            assert!(
                output_path.exists(),
                "Output file not created for format {}",
                format
            );
            let content = fs::read_to_string(&output_path).unwrap();
            assert!(
                !content.is_empty(),
                "Output file empty for format {}",
                format
            );
            ...
            match format {
                ...
                &"yaml" => {
                    assert!(
                        content.contains(':') && content.contains("target_hue"),
        ...
        for (format, result, _) in &results[1..] {
            assert_eq!(
                result.matched_colors.len(),
                base_result.matched_colors.len(),
                "Color count inconsistent for format {}",
                format
            );
            assert_eq!(
                result.total_processed, base_result.total_processed,
                "Processing count inconsistent for format {}",
                format
            );
        }

        println!(
            "✓ Output format consistency validated across {} formats",
            formats.len()
        );
    }
...
"#;
        ...
        let analysis = result.unwrap();
        assert!(
            analysis.matched_colors.len() <= 100,
            "Config limit not respected"
        );

        println!(
            "✓ Configuration integration successful: {} colors matched",
            analysis.matched_colors.len()
        );
    }
    ...
    fn execute_hue_analysis_workflow(
        args: HueArgs,
    ...
    fn apply_hue_filters(
        colors: &[Color],
        args: &HueArgs,
    ) -> Result<Vec<Color>, Box<dyn std::error::Error>> {
        let filtered: Vec<Color> = colors
            .iter()
            .filter(|color| {
                let hsl = color.to_hsl();
    ...
    fn sort_and_limit_colors(
        ...
        if let Some(sort_by) = &args.sort_by {
            match sort_by.as_str() {
                "hue" => {
                    colors.sort_by(|a, b| a.to_hsl().hue.partial_cmp(&b.to_hsl().hue).unwrap())
                }
                ...
                "saturation" => colors.sort_by(|a, b| {
                    a.to_hsl()
                        .saturation
                        .partial_cmp(&b.to_hsl().saturation)
                        .unwrap()
                }),
                "lightness" => colors.sort_by(|a, b| {
                    a.to_hsl()
                        .lightness
                        .partial_cmp(&b.to_hsl().lightness)
                        .unwrap()
                }),
                "name" => {
