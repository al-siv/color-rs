...
use color_rs::color_ops::analysis::hue::{
    calculate_hue_distance, normalize_hue, meets_criteria, SortCriteria, HueAnalysisOptions,
    HueAnalysisResult, ColorCollectionType, analyze_collection_hues
};
...
fn test_analyze_collection_hues_with_filters() -> color_rs::Result<()> {
    ...
    for result in &results {
        assert!(result.saturation >= 50.0, "Saturation {} should be >= 50.0", result.saturation);
        assert!(result.lightness >= 30.0, "Lightness {} should be >= 30.0", result.lightness);
    }
