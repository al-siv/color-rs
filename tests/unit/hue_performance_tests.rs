...
mod hue_performance_tests {
    ...
    fn test_hue_distance_performance() {
        ...
        for (name, iterations) in test_cases {
            ...
            println!(
                "Hue distance {} iterations: {:?} total, {} ns/op",
            ...
            assert!(
                per_operation < 100,
                "Hue distance calculation too slow: {} ns/op",
                per_operation
            );
        }
    }
    ...
    fn test_hue_normalization_performance() {
        ...
        println!(
            "Hue normalization {} operations: {:?} total, {} ns/op",
            test_values.len(),
            elapsed,
            per_operation
        );
        ...
        assert!(
            per_operation < 50,
            "Hue normalization too slow: {} ns/op",
            per_operation
        );
    }
    ...
    fn test_collection_filtering_performance() {
        ...
        for (size_name, colors) in collections {
            ...
            println!(
                "Collection filtering {} colors: {:?} avg, {} ns/color",
                colors.len(),
                avg_duration,
                per_color
            );
            ...
            assert!(
                result_variance < 1.0,
                "Results not consistent across runs: variance {}",
                result_variance
            );
        }
    }
    ...
    fn test_analysis_pipeline_performance() {
        ...
        for (scenario_name, tolerance) in test_scenarios {
            ...
            println!(
                "Hue analysis pipeline {} ({}): {} ms/analysis",
            ...
            assert!(
                per_analysis < 10,
                "Analysis pipeline too slow for {}: {} ms",
                scenario_name,
                per_analysis
            );
        }
    ...
    fn test_memory_efficiency() {
        ...
        println!(
            "Memory efficiency - Original: {}, Filtered: {}, Ratio: {:.2}",
            initial_colors.len(),
            filtered.len(),
            efficiency_ratio
        );
        ...
        assert!(
            efficiency_ratio >= 0.1 && efficiency_ratio <= 0.4,
            "Memory efficiency outside expected range: {:.2}",
            efficiency_ratio
        );
    }
    ...
    fn test_concurrent_operations_performance() {
        ...
        println!(
            "Concurrent operations {} threads × {} ops: {:?} total, {} ms/op",
        ...
        assert!(
            per_operation < 5,
            "Concurrent operations too slow: {} ms/op",
            per_operation
        );
    }
    ...
    fn test_extreme_collection_performance() {
        ...
        println!(
            "Starting extreme collection test with {} colors",
            extreme_size
        );
        ...
        println!(
            "Extreme collection filtering: {:?}, {} matches",
            elapsed,
            filtered.len()
        );
        ...
        assert!(
            elapsed.as_secs() < 5,
            "Extreme collection test took too long: {:?}",
            elapsed
        );
    }
    ...
    fn perform_full_hue_analysis(
        ...
        let hue_range = if filtered.len() > 1 {
            let hues: Vec<f64> = filtered.iter().map(|c| c.to_hsl().hue).collect();
