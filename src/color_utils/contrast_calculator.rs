...
use palette::{Lab, Srgb, IntoColor, color_difference::Ciede2000};
    ...
    pub fn get_comprehensive_assessment(
        ...
        let overall_level = if analyses.len() > 1 {
            ...
            match (primary_level, secondary_level) {
                ...
                (ContrastLevel::Marginal, _) | (_, ContrastLevel::Marginal) => ContrastLevel::Marginal,
...
fn generate_recommendation(analyses: &[ContrastAnalysis]) -> String {
    ...
    match primary.contrast_level {
        ContrastLevel::High => "Excellent contrast - suitable for all text sizes and purposes.".to_string(),
        ContrastLevel::Medium => "Good contrast - meets WCAG AA standards for normal text.".to_string(),
        ContrastLevel::Marginal => "Marginal contrast - only suitable for large text or decorative purposes.".to_string(),
        ContrastLevel::Low => "Poor contrast - not suitable for text. Consider using higher contrast colors.".to_string(),
    }
}
...
mod tests {
    ...
    fn test_enhanced_calculator() {
        ...
        let results = calculator.calculate_enhanced_contrast(white, black).unwrap();
        assert_eq!(results.len(), 2); // Primary + secondary
        
        let assessment = calculator.get_comprehensive_assessment(white, black).unwrap();
