...
use crate::gradient_config::{GradientConfig, ColorPair};
...
pub fn validate_functional_pattern_performance() -> Result<(), String> {
    ...
    for _ in 0..iterations {
        for &lab1 in &test_data.lab_colors {
            for &lab2 in &test_data.lab_colors {
                ...
                let _dist1 = black_box(DistanceAlgorithm::DeltaE2000.calculate_distance(lab1, lab2));
                let _dist2 = black_box(DistanceAlgorithm::EuclideanLab.calculate_distance(lab1, lab2));
    ...
    let performance_ratio = enum_dispatch_time.as_nanos() as f64 / direct_call_time.as_nanos() as f64;
...
pub fn validate_allocation_elimination() -> Result<(), String> {
    ...
    let test_data = PerformanceTestData::new();
    let pairs: Vec<(ValidatedLab, ValidatedLab)> = test_data.lab_colors
        .iter()
    ...
    let _distances = black_box(
        DistanceAlgorithm::DeltaE2000.calculate_distances(&pairs)
    );
...
pub fn validate_zero_cost_abstractions() -> Result<(), String> {
    ...
    for _ in 0..100_000 {
        ...
        let _distance = black_box(
            DistanceAlgorithm::DeltaE2000.calculate_distance(lab1, lab2)
        );
    }
...
pub fn validate_memory_usage_patterns() -> Result<(), String> {
    ...
    let _matrix = black_box(
        DistanceAlgorithm::DeltaE2000.calculate_distance_matrix(colors)
    );
