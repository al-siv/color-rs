...
fn parse_ral_classic(md_content: &str) -> Vec<RalClassicColor> {
    ...
    for line in md_content.lines() {
        ...
        if line.is_empty()
            || line.starts_with('|') && (line.contains("RAL number") || line.contains("----"))
        {
            continue;
        }
        ...
        if line.starts_with("| RAL ") {
            ...
            if columns.len() >= 19 {
                ...
                if colors.len() < 3 {
                    println!(
                        "Parsed RAL Classic color {}: {} - {} - {} (L*:{}, a*:{}, b*:{}, CMYK:{},{},{},{}, LRV:{})",
                        colors.len() + 1,
                        code,
                        name,
                        hex,
...
fn parse_ral_design(md_content: &str) -> Vec<RalDesignColor> {
    ...
    for line in md_content.lines() {
        ...
        if line.is_empty()
            || line.starts_with('|')
                && (line.contains("Name") || line.contains("CIELAB") || line.contains("----"))
        {
            continue;
        }
        ...
        if line.starts_with('|') && !line.contains("Hue") {
            ...
            if columns.len() >= 9 {
                ...
                let hue = columns[2]
                    ...
                    .parse::<f32>()
                    .unwrap_or(0.0);
                let lightness = columns[3]
                    .trim_end_matches('%')
                    .parse::<f32>()
                    .unwrap_or(0.0);
                let chromaticity = columns[4]
                    .trim_end_matches('%')
                    .parse::<f32>()
                    .unwrap_or(0.0);
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let row_count = ral_classic_content
        .lines()
        .filter(|line| line.trim().starts_with("| RAL "))
        .count();
    ...
    let design_row_count = ral_design_content
        .lines()
        .filter(|line| {
            let trimmed = line.trim();
            trimmed.starts_with('|')
                && !trimmed.contains("Name")
                && !trimmed.contains("CIELAB")
                && !trimmed.contains("----")
                && !trimmed.contains("Hue")
        })
        .count();
    ...
    println!(
        "Parsed {} RAL Design System+ colors",
        ral_design_colors.len()
    );
