...
use super::ral_matcher::{RalMatch, RalClassification};
...
fn color_match_to_ral_match(color_match: &NewColorMatch, classification: RalClassification) -> RalMatch {
    RalMatch {
        code: color_match.entry.metadata.code.clone().unwrap_or_default(),
        name: color_match.entry.metadata.name.clone(),
        hex: color_match.entry.metadata.extra_data.get("hex")
            .cloned()
            .unwrap_or_else(|| format!("#{:02x}{:02x}{:02x}", 
...
pub fn find_closest_ral_classic_compat(rgb: &crate::color_parser::RgbColor, max_results: usize) -> Vec<RalMatch> {
...
pub fn find_closest_ral_design_compat(rgb: &crate::color_parser::RgbColor, max_results: usize) -> Vec<RalMatch> {
    let rgb_array = [rgb.r, rgb.g, rgb.b];
...
pub fn find_closest_ral_colors_compat(rgb: &crate::color_parser::RgbColor, max_results: usize) -> Vec<RalMatch> {
    let rgb_array = [rgb.r, rgb.g, rgb.b];
    ...
    all_matches.extend(classic_matches.iter()
        .map(|m| color_match_to_ral_match(m, RalClassification::Classic)));
    
    all_matches.extend(design_matches.iter()
        .map(|m| color_match_to_ral_match(m, RalClassification::DesignSystem)));
...
pub fn find_ral_by_code_compat(code: &str) -> Option<RalMatch> {
    if let Some((collection_name, entry)) = UNIFIED_MANAGER.find_by_code(code) {
        ...
        Some(RalMatch {
            ...
            hex: entry.metadata.extra_data.get("hex")
                .cloned()
                .unwrap_or_else(|| format!("#{:02x}{:02x}{:02x}", 
                    entry.color.rgb[0],
                    entry.color.rgb[1],
...
mod tests {
    ...
    fn test_backward_compatibility() {
        ...
        assert!(classic_matches.iter().all(|m| m.classification == RalClassification::Classic));
        
        // Test design compatibility
        let design_matches = find_closest_ral_design_compat(&red, 2);
        assert!(!design_matches.is_empty());
        assert!(design_matches.iter().all(|m| m.classification == RalClassification::DesignSystem));
