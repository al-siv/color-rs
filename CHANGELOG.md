...
- Deprecated compatibility module `src/compat.rs` removed early (original 0.21.0 target) as part of Milestone 3 Batch 3. Migration: replace any legacy factory/trait usage with direct parsing (`parse_color`, `get_color_name`) and command execution (`execute_command` with `ExecutionContext`). Rationale: eliminates transitional shim; reduces surface area and maintains single source APIs.
- Removed deprecated no-op `ColorFormatter::format_comprehensive_report`; use `collect_color_analysis_data` + downstream formatting utilities for composing output lines.

### Deprecated
- (None currently) — all previously scheduled removals executed in this cycle.
