...
fn main() -> Result<()> {
    ...
    println!("Red in LAB: Lab({:.1}, {:.1}, {:.1})", red_lab.l, red_lab.a, red_lab.b);
    println!("Blue in LAB: Lab({:.1}, {:.1}, {:.1})", blue_lab.l, blue_lab.a, blue_lab.b);
    ...
    let intelligent_stops = GradientCalculator::calculate_intelligent_stops_integer(
        8, 0.9, 0.1, 0, 100
    );
    println!("Intelligent stops: {:?}", intelligent_stops);
    
    let equal_stops: Vec<u8> = (0..8)
        .map(|i| (i * 100 / 7) as u8)
        .collect();
