...
fn main() -> Result<()> {
    ...
    println!("   Colors: {} → {}", basic_gradient.colors().start(), basic_gradient.colors().end());
    ...
    println!("   Linear: ease_in={:.2}, ease_out={:.2}", 
        linear.easing().ease_in_value(), linear.easing().ease_out_value());
    
    let smooth = smooth_gradient("green", "yellow")?;
    println!("   Smooth: ease_in={:.2}, ease_out={:.2}", 
        smooth.easing().ease_in_value(), smooth.easing().ease_out_value());
    ...
    println!("   - Colors: {} → {}", complex_gradient.colors().start(), complex_gradient.colors().end());
    println!("   - SVG output: {:?}", complex_gradient.image_output().svg_filename());
    println!("   - PNG output: {:?}", complex_gradient.image_output().png_filename());
    println!("   - Width: {}", complex_gradient.image_output().width());
    println!("   - Legend: {}", complex_gradient.image_output().show_legend());
    ...
    println!("   - Custom easing: {:.2} / {:.2}", 
        advanced.easing().ease_in_value(), advanced.easing().ease_out_value());
    println!("   - Position range: {}%-{}%", 
        advanced.position_range().start(), advanced.position_range().end());
    println!("   - Both image outputs with {}px width", advanced.image_output().width());
    if let Some(file_output) = advanced.file_output() {
        println!("   - File output: {} format to {}", 
            format!("{:?}", file_output.format()), file_output.filename());
    }
...
mod tests {
    ...
    fn test_immutability_demonstration() {
        ...
        let modified = original.clone()
            .with_width(800).unwrap()
            .with_steps(10).unwrap();
        
        // Original should be unchanged
        assert_eq!(original.image_output().width(), 1000); // Default width
        assert!(matches!(original.stop_config(), StopConfig::IntelligentStops(5))); // Default stops
    ...
    fn test_error_propagation_in_chain() {
        ...
        let result = GradientConfig::basic("#FF0000", "#0000FF")
            .and_then(|c| c.with_width(0)); // Invalid width
