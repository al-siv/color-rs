...
use color_rs::color_parser::{
    ColorCollection, CssColorCollection, SearchFilter, UnifiedColorManager, UniversalColor,
};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Some((collection, entry)) = manager.find_by_code("RAL 1000") {
        println!(
            "   RAL 1000: {} from {} collection",
            entry.metadata.name, collection
        );
